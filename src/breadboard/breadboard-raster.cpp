...
QRect Breadboard::getRasterBounds() {
	return getDistortedRect(BB_ROW_X,BB_ROW_Y,BB_ONE_ROW*iconSizeMinimum(),((BB_INDEXES*2)+1)*iconSizeMinimum());
}

bool Breadboard::isOnRaster(QPoint pos) {
	return getDistortedRect(BB_ROW_X,BB_ROW_Y,BB_ONE_ROW*iconSizeMinimum(),BB_INDEXES*iconSizeMinimum()).contains(pos)
			|| getDistortedRect(BB_ROW_X,BB_ROW_Y+(BB_INDEXES+1)*iconSizeMinimum(),BB_ONE_ROW*iconSizeMinimum(),BB_INDEXES*iconSizeMinimum()).contains(pos);
}
...
QRect Breadboard::getDistortedRect(unsigned x, unsigned y, unsigned width, unsigned height) {
       return QRect(getDistortedPosition(QPoint(x,y)),getDistortedSize(QSize(width,height)));
}

QRect Breadboard::getDistortedGraphicBounds(QImage buffer, unsigned scale) {
       return getDistortedRect(buffer.offset().x(), buffer.offset().y(), buffer.width()*scale, buffer.height()*scale);
}

QPoint Breadboard::getDistortedPosition(QPoint pos) {
       return QPoint (pos.x()*width()/minimumWidth(), pos.y()*height()/minimumHeight());
}

QSize Breadboard::getDistortedSize(QSize minimum) {
       return QSize(minimum.width()*width()/minimumWidth(), minimum.height()*height()/minimumHeight());
}

QPoint Breadboard::getMinimumPosition(QPoint pos) {
       return QPoint(pos.x()*minimumWidth()/width(), pos.y()*minimumHeight()/height());
}
